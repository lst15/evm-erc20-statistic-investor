...
class TransactionsIOUseCase{
  ...
  async exec({main_contracts,token_pair,user_address}:TransactionsIOUseCaseRequest){

    const pair_in_filter = main_contracts.weth_contract.filters.Transfer(token_pair)
    const pair_out_filter = main_contracts.weth_contract.filters.Transfer(null,token_pair)
    ...
    const pair_transactions_in = await main_contracts.weth_contract.queryFilter(pair_in_filter)
    const pair_transactions_out = await main_contracts.weth_contract.queryFilter(pair_out_filter)

    const contract_transactions_in = await main_contracts.token_contract.queryFilter(contract_in_filter)
    const contract_transactions_out = await main_contracts.token_contract.queryFilter(contract_out_filter)
