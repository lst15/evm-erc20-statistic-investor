...
class TransactionsIOUseCase {
  ...
  async exec({
    ...
    token_pair,
    user_address,
  }: TransactionsIOUseCaseRequest): Promise<TransactionIOModel> {
    const approve_filter =
      main_contracts.token_contract.filters.Approval(user_address);

    const pair_in_filter = main_contracts.weth_contract.filters.Transfer(
      null,
      token_pair
    );
    const pair_out_filter = main_contracts.weth_contract.filters.Transfer(
      token_pair,
      null
    );

    const contract_in_filter = main_contracts.token_contract.filters.Transfer(
      null,
      user_address
    );
    const contract_out_filter =
      main_contracts.token_contract.filters.Transfer(user_address);
    console;
    try {
      const pair_transactions_in =
        (await main_contracts.weth_contract.queryFilter(
          pair_in_filter
        )) as EventLog[];
      ...
      const contract_transactions_in =
        (await main_contracts.token_contract.queryFilter(
          contract_in_filter
        )) as EventLog[];
