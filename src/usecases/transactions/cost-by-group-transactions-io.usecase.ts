...
class CostByGroupTransactionsIOUseCase {
  ...
  async exec({ groups, transaction_io, main_contract }: CostByGroupTransactionsIOUseCaseRequest) {
    ...
    for (var i in groups) {
      ...
      for (var _ in groups[index]) {
        ...
        if (group.operation == "buy") {
          ...
          const tokens_sell_transaction = transaction_io.contract_transactions?.contract_transactions_in.filter(transaction => transaction.transactionHash == group.transactionHash) as EventLog[]
          ...
          const current_tokens = await main_contract.token_contract.balanceOf(env.USER_ADDRESS, { blockTag: group.blockNumber })
